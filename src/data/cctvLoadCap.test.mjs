...
// The per-area load cap at the route: at most 2,500 cameras within 50 km of a
...
test('an area lists its cameras nearest first with distance, region and lookup state', async (t) => {
  ...
  assert.deepEqual(
    { ...body.area },
    { lat: 29.76, lon: -95.37, radiusKm: 50, limit: 2500, inArea: 2, loaded: 2, dropped: 0, reachKm: 50, capped: false, total: 3, generation: 1, pending: [] },
  );
...
test('6,000 cameras in one state are all reachable, 2,500 at most per area', async (t) => {
  const root = setup(t, [
    ...cluster('hou', HOUSTON, 2000),
    ...cluster('dal', DALLAS, 2000),
    ...cluster('elp', EL_PASO, 2000),
  ...
  for (const center of [HOUSTON, DALLAS, EL_PASO]) {
    ...
    assert.equal(body.sources.length, 2000);
  ...
  assert.equal(dense.sources.length, 2500);
  assert.equal(dense.area.inArea, 3000);
  assert.equal(dense.area.loaded, 2500);
  assert.equal(dense.area.dropped, 500);
  assert.equal(dense.area.capped, true);
  assert.ok(dense.area.reachKm < 50);
  assert.equal(dense.area.total, 9000);
