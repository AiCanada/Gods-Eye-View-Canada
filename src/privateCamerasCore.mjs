 ...
 * one login: a Home Assistant / Scrypted bridge for Arlo cameras, or one shared
...
export const PRIVATE_CAMERA_KINDS = Object.freeze([
  Object.freeze({
    ...
    unlocks:
      'Your Arlo cameras in the CCTV panel and on the map, through a local Home Assistant (hass-aarlo) or Scrypted bridge, or the GEV Arlo Feed Relay browser extension, which passes along the latest clip pictures from your own signed-in my.arlo.com feed. This app never signs in to Arlo itself.',
    ...
    provider: 'Arlo via local bridge',
  }),
  Object.freeze({
    id: 'business',
...
/** Longest Arlo camera name a relay camera may be matched by. */
const RELAY_CAMERA_NAME_LIMIT = 60;
/** What the CCTV panel names as the source of a relay camera's pictures. */
const RELAY_PROVIDER = 'Arlo browser feed relay (clip pictures)';
...
/**
 * Arlo's own website and cloud (my.arlo.com and the rest of arlo.com). It is a
 * sign-in page and app, never a camera picture, so it cannot be a bridge or a
 * snapshot address; the app never sends it a login.
 */
export function isArloCloudUrl(value) {
  try {
    const host = new URL(String(value || '')).hostname.toLowerCase();
    return host === 'arlo.com' || host.endsWith('.arlo.com') || host.endsWith('.arlo.netgear.com');
  } catch {
    return false;
  }
}

/** Whether a site points at Arlo's website instead of a local bridge or snapshot address. */
export function siteUsesArloWebsite(site) {
  if (isRelaySite(site)) return false;
  return Boolean(isArloCloudUrl(site?.bridgeUrl) || site?.cameras?.some((camera) => isArloCloudUrl(camera.source)));
}

/**
 * A home site whose pictures come from the GEV Arlo Feed Relay: a browser
 * extension that, while the owner's own signed-in my.arlo.com feed is open,
...
/**
 * How a camera name from the Arlo feed and a camera name typed in POWER UP are
 ...
 * http(s) address or a Home Assistant entity) rather than being an Arlo name.
 */
...
/**
 * The Arlo feed name a relay camera answers to: its Arlo name override when one
 * is saved as plain text, otherwise its own name. A saved address or entity
 * (from the site's bridge setup, or an older my.arlo.com source) is never a name.
 */
...
/** The first Arlo name two cameras of a relay site would both answer to, or ''. */
 ...
 * source there is the camera's name in the Arlo feed when that differs from the
...
export function applyPrivateCameraUpdate(body, previous = emptyPrivateCameraConfig()) {
  ...
  if (body.cameras !== undefined) {
    ...
    for (const [index, row] of body.cameras.entries()) {
      ...
      if (relay) {
        // Optional: the camera's name in the Arlo feed when it differs from its name here.
        // A saved bridge source (a snapshot or my.arlo.com address, or a Home Assistant
        ...
        if (source && !legacyBridgeSource) {
          if (isHttpUrl(source)) return { ok: false, error: `${where} (${name}) Arlo name must be the camera name shown in the Arlo feed, not an address` };
          if (ENTITY_PATTERN.test(source)) return { ok: false, error: `${where} (${name}) Arlo name must be the camera name shown in the Arlo feed, not a Home Assistant entity` };
          if (source.length > RELAY_CAMERA_NAME_LIMIT || CONTROL_CHARS.test(source)) return { ok: false, error: `${where} (${name}) Arlo name is not valid` };
          // An Arlo name equal to the camera's own name is no override: typing the Name clears a saved one.
  ...
  // Each Arlo feed name reaches exactly one camera of a relay site.
  if (relay) {
    const duplicate = duplicateRelayMatchName(next.cameras);
    if (duplicate) return { ok: false, error: `Two cameras would both match the Arlo camera “${duplicate}”` };
  }
      ...
      sites: normalized.sites
        ...
        .map((site) => {
          ...
          return {
            ...
            arloWebsite: siteUsesArloWebsite(site),
            ...
            cameras: site.cameras.map((camera) => ({
              ...
              arloWebsite: !relaySite && isArloCloudUrl(camera.source),
 ...
 * Arlo name its pictures arrive under instead: nothing is fetched for it.
