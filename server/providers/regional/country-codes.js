/**
 * Country codes: ISO 3166-1 alpha-2 | alpha-3 | UN M49 numeric | UN short name.
 *
 * Generated once from the United Nations Statistics Division's M49 standard
 * (https://unstats.un.org/unsd/methodology/m49/overview/), the table the UN's own
 * databases key their areas by; the alpha-3 codes were cross-checked against
 * the World Bank's country list. The UN data services are asked by M49 code,
 * the World Health Organization by alpha-3, and the app knows a place by
 * alpha-2, so all three are needed. An area the UN table does not list (Taiwan,
 * Kosovo) is not here.
 */
const ROWS = [
  'AD|AND|20|Andorra',
...
/** @type {Map<string, {iso2: string, iso3: string, m49: number, name: string}>} */
const BY_ISO2 = new Map(
  ROWS.map((row) => {
    const [iso2, iso3, m49, name] = row.split('|');
    return [iso2, Object.freeze({ iso2, iso3, m49: Number(m49), name })];
  }),
);

/** The codes of one country, or null. */
export function countryCodes(iso2) {
  return (
    BY_ISO2.get(
      String(iso2 || '')
        .trim()
...
export const COUNTRY_COUNT = BY_ISO2.size;
