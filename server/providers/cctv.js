...
export function cctvProxy({ sourceRoot = process.cwd() } = {}) {
  ...
  /** Region-capped view of each catalogue snapshot, computed once per snapshot
   * and limit rather than on every /sources request. */
  const regionCapped = new WeakMap();
  const capByRegion = (sources, limit) => {
    const cached = regionCapped.get(sources);
    if (cached?.limit === limit) return cached.result;
    const result = capSourcesPerRegion(sources, limit);
    regionCapped.set(sources, { limit, result });
    return result;
  };
    ...
    server.middlewares.use('/api/cctv', async (req, res) => {
      try {
        ...
        if (url.pathname === '/sources') {
          // Region cap: at most N cameras per Canadian province or territory,
          // per US state, and per country elsewhere, unless the viewer turned
          // it off. Cameras outside it are never listed, so the browser never
          // fetches their frames.
          const { limit, enabledByDefault } = cctvRegionCapSettings();
          const capOn = regionCapRequested(
            url.searchParams.get('regionCap'),
            enabledByDefault,
          );
          const { kept: served, dropped } = capOn
            ? capByRegion(sources, limit)
            : { kept: sources, dropped: new Map() };
          const body = {
            regionCap: {
              enabled: capOn,
              limit,
            ...
            sources: served.map((source) => ({
              id: source.id,
              name: source.name,
              city: source.city,
              cityId: source.cityId,
              country: source.country || '',
              region: cctvRegionKey(source),
              provider: source.provider,
              lat: source.lat,
              lon: source.lon,
          ...
          try {
            ...
            if (
              ...
              setHealth(cameraId, {
                ...
                label: source?.provider || 'Configured source',
                message:
                  'HLS playlist proxied; playback depends on browser support',
              });
            } else {
              setHealth(cameraId, {
                status: 'ok',
        ...
        const resolvedUrl =
          !backingOff && source?.frameResolver
            ? await resolveFrameUrl(source)
            : '';
        const videoFeed = isVideoFeedType(normalizeFeedType(source?.feedType));
        const upstreamCandidate =
          resolvedUrl || source?.snapshotUrl || (!videoFeed ? source?.url : '');
        ...
        const browserDirect = Boolean(source && browserDirectImageUrl(source));
        const skipFallbacks =
          backingOff || browserDirect || (videoFeed && !upstreamCandidate);

        const upstreamImage = skipFallbacks
          ? null
